                        ...
                        <div class="col-md-12">
                            <div class="row">
                                ...
                                <div class="col-sm-2" style="margin-top: 2%;">
                                    <button class="form-control btn btn-primary" onclick="searchWajib()" style="color: white;">Cari</button>
                                </div>
                                <div class="col-sm-2" style="margin-top: 2%;">
...
<script>
    ...
    function searchWajib() {
        $.ajax({
            url: "<?php echo base_url("LaporanController/load_data_wajib"); ?>",
            type: "POST",
            data: {
                id_user: $("#id_user").val(),
                dateOne: $("#dateOne").val(),
            ...
            success: function(res) {
                ...
                for (i = 0; i < obj.data.length; i++) {
                    html += '<tr>' +
                        ...
                        '<td>' + obj.data[i].tgl_bayar + '</td>' +
                        '<td>' + obj.data[i].nama_bulan + '</td>' +

                        '<td>' + '' +
                        ((obj.data[i].status == 200) ?
                            'Berhasil' :
                            (obj.data[i].status == 201) ?
                            'Pending' : 'Gagal') +
                        '</td>' +
                        '<td>' + 'Rp. ' + number_format(obj.data[i].nominal) + '</td>' +
                        '</tr>';
                }
                $('#show_data').html(html);
    ...
    function exportPdf() {
        ...
        $.ajax({
            url: "<?php echo base_url("/LaporanController/pdf_wajib"); ?>",
            type: "POST",
            data: {
                id_user: id_user,
                dateOne: dateOne,
            ...
            success: function(res) {
                ...
                var url = "<?= base_url("/LaporanController/pdf_wajib_view") ?>" + '?id_user=' + obj.id_user + '&dateOne=' + obj.dateOne + '&dateTwo=' + obj.dateTwo + '';
